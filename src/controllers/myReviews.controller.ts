...
export const getPending = asyncHandler(async (req: Request, res: Response) => {
  ok(res, await myReviewsService.getPendingReviews(uid(req), req.query));
});

export const getPast = asyncHandler(async (req: Request, res: Response) => {
  ok(res, await myReviewsService.getPastReviews(uid(req), req.query));
});
...
export const getReviewOrderData = asyncHandler(
  async (req: Request, res: Response) => {
    ok(
      res,
      await myReviewsService.getReviewOrderData(uid(req), req.params.orderId),
    );
  },
);
